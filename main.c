...
static int __init blk_init(void)
{
	...
	// addr_tree = kzalloc(sizeof(node *), GFP_KERNEL);
...
static void shift_submit_bio(struct bio *bio)
{
	...
	struct bio *clone = bio_alloc_clone(current_bdev_handle->bdev, bio, GFP_KERNEL, bio->bi_pool);
	...
	{
		node *new = create_node(bio->bi_iter.bi_sector, last_sector);
		pr_info("newnnn\n");
		//addr_tree = 
		insert_b(addr_tree, new);
		clone->bi_iter.bi_sector = last_sector;
		last_sector += 8;
	}
	pr_info("redirected from %llu, to %llu\n", bio->bi_iter.bi_sector, clone->bi_iter.bi_sector);
	submit_bio(clone);
}
...
static struct gendisk *shift_blkdev_create(char *shift_name)
{
	...
	{
		pr_info("you ve already created shift change disk \n");
		return shift_disk;
	}
}
